...
describe('MessageList.test.js', () => {
  ...
  beforeEach(() => {
    ...
    */
    const mensajeWrapper = {
      render(h) {
        return h(Message, { props: { Message: 'Hola' } })
      }
    }
    cmp = mount(MessageList, {
      // Be aware that props is overridden using `propsData`
      slots: {
        default: mensajeWrapper
      }
    })
  })
  it('Los Mensajes se colocan en el componente ListaMensajes', () => {
    const list = cmp.find(MessageList)
    expect(list.find(Message).isVueInstance()).toBe(true)
  })

  it('El slot header renderiza el texto por defecto', () => {
    const header = cmp.find('.list-header');
    expect(header.text().trim()).toBe('Este es el header')
  })

  it('El slot header renderiza EN .list-header', () => {
